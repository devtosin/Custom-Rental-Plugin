...
(function ($) {
    ...
    function addressWidget(cfg) {
        ...
        function tryTypedPostcodeFallback() {
            var raw = $inp.val().trim();
            var m = raw.match(/\b([A-Z]{1,2}\d[A-Z\d]?\s*\d[A-Z]{2})\b/i);
            if (!m) return false;
            var pc = m[1].toUpperCase().replace(/\s+/g, '');
            if (inLondon(pc)) {
                setFb(true, '\u2713 We deliver to this postcode');
                markState(true, pc, raw);
            } else {
                setFb(false, 'Sorry — <strong>' + esc(pc) + '</strong> is outside our Greater London zone.');
                markState(false, '', raw);
            }
    ...
    function initForm($wrap) {
        ...
        function handlePickupDateChanged(ds, selectedDateObj) {
            pickupDate = ds;
            if (pickupDate && returnFP) {
                var min = new Date(selectedDateObj || pickupDate);
                min.setDate(min.getDate() + (cfg.min_days || 1));
                if (typeof returnFP.set === 'function') returnFP.set('minDate', min);
                if (typeof returnFP.clear === 'function') returnFP.clear();
                else {
                    dropoffDate = '';
                    $wrap.find('#lc-dropoff-date').val('');
                }
            }
        ...
        function handleDropoffDateChanged(ds) {
            dropoffDate = ds;
        ...
        if (typeof flatpickr === 'function') {
            flatpickr($wrap.find('#lc-pickup-date')[0], {
                ...
                onChange: function (sel, ds) {
                    handlePickupDateChanged(ds, sel.length ? sel[0] : null);
                }
            });
        } else {
            var $pickup = $wrap.find('#lc-pickup-date');
            $pickup.prop('readonly', false).attr('type', 'date').attr('min', today.toISOString().slice(0, 10));
            $pickup.on('change', function () {
                handlePickupDateChanged($(this).val(), $(this).val());
            });
        }
        ...
        if (typeof flatpickr === 'function') {
            returnFP = flatpickr($wrap.find('#lc-dropoff-date')[0], {
                ...
                altFormat:  'D, d M Y',
                onChange: function (sel, ds) {
                    handleDropoffDateChanged(ds);
                }
            });
        } else {
            var $dropoff = $wrap.find('#lc-dropoff-date');
            $dropoff.prop('readonly', false).attr('type', 'date');
            $dropoff.on('change', function () {
                handleDropoffDateChanged($(this).val());
            });
            returnFP = {
                set: function (key, val) {
                    if (key === 'minDate') {
                        var vv = (val instanceof Date) ? val.toISOString().slice(0, 10) : String(val).slice(0, 10);
                        $dropoff.attr('min', vv);
                    }
                },
                clear: function () { $dropoff.val(''); }
            };
        }
